...
def scan_library(dirname, typ, device, reg):
    ...
    if rd.exists() and "Generated by [Moxygen]" in rd.read_text(errors="replace"): rem.append("README is moxygen output")
    row["moxygen remnants"] = "OK" if not rem else ", ".join(rem)
    row["_docs/ site config"] = ("OK" if all((d / "_docs" / f).exists() for f in ("_config.yml", "Gemfile", "_data/navigation.yml")) else "MISSING")
    if slug and (d / ".github/workflows/docs.yml").exists():
        pg = gh(f"repos/{slug}/pages", ".source.branch")
        row["Pages enabled"] = "OK" if pg == "gh-pages" else ("NOT ENABLED" if pg in ("", "?") else f"branch {pg}")
    else:
        row["Pages enabled"] = "—"

...
| 14 | Build identity DONE in code 2026-09-23 (except Apis, agent C's repo): `<LIB>_LIBRARY_VERSION` + `<LIB>_LIBRARY_COMMIT` (blank in IDE builds) in NW_Core b7f0e55, Walrus 2fce354, Haar 38c7360, Libelle 2f89106, Margay 1fbe8ec (+ `SKETCH_COMMIT`); status row is now `Time,Trigger,Device,Serial,HW,FW,FWCommit,Lib,LibCommit,Code,Note,Page0,Page1,Page2`; firmware `FW_COMMIT` → served Page 0 0x18–0x1B + dirty bit 0x1C in Project-Walrus ff162f2, Haar 1950642, Libelle 315d8ed (EEPROM zeros; 0x0B–0x0D kept). Remaining: Apis library + firmware (after agent C), the spec's Block 3 paragraph + LIBRARY-DESIGN column list (after agent C frees the spec repo), NW-Build injecting the defines (agent D) | Apis, spec, NW-Build | agents C and D | hardware-untested |

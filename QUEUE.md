...
| 9 | Review how Margay (and therefore Okapi) writes files to the SD card overall: open/close per row, folder layout, the header path, the SdFat version, error handling | Margay_Library, Okapi_Library | Andy's decision on the layout (proposal of 2026-09-23: drop NW/ and Logs/, `/<SN>/Log00001.txt` beside `/<SN>/Sta00001.txt`, header row first) | Read 2026-09-23: both write `/NW/<SN>/Logs/Log<n>.txt` (Margay 5 digits, Okapi 2), `HWTest.txt` in `/NW/<SN>/`, first line `Lib = x SN = y` before the header, three chdir + open + close per row; status-file writing joins here |

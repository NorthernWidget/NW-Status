...
| B4 | Status file: the device rows. RULE (Andy 2026-09-23): tell a Margay wake from a sensor powering back up. Margay cuts the sensor rail every sleep, so the reset notice (0xE6) that begin() captures right after the rail comes up is expected and makes no row; a reset the library captures later in the same wake, with the rail still on, is a sensor restarting on its own (brownout, watchdog) and makes a row. The logger's own boot row is written once, at its first wake. Margay writes `sta<n>.csv` with its own boot row (10a1650) and offers `statusStr()`; what remains is the sketch or library call that writes `time,report,<printStatus()>` whenever a device's `reportKind()` is not zero, and the boot rows for the devices | [Margay_Library #7](https://github.com/NorthernWidget/Margay_Library/issues/7) | Andy's word on where the call lives (sketch template vs. Margay) |
